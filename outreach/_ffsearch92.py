# -*- coding: utf-8 -*-
"""Search ff index (batch92) by keyword; rank by hits, show snippet + tracker usage."""
import json, sys, io, collections
sys.stdout = io.TextIOWrapper(sys.stdout.buffer, encoding='utf-8')
IDX = json.load(open(r'C:/Users/micme/Desktop/micmer/futuro fortissimo/outreach/_ffidx_b92.json', encoding='utf-8'))
...
for code, v in IDX.items():
    ...
    if score:
        ...
        snip = v['content'][max(0, pos - 150):pos + 400].replace('\n', ' ')
        rows.append((score, CNT.get(code, 0), code, v['title'], snip))
rows.sort(key=lambda r: (CNT.get(r[2],0), -r[0]))
