# -*- coding: utf-8 -*-
"""Search ff index (batch93) by keyword; rank by tracker usage then hits."""
import json, sys, io, collections
sys.stdout = io.TextIOWrapper(sys.stdout.buffer, encoding='utf-8')
IDX = json.load(open(r'C:/Users/micme/Desktop/micmer/futuro fortissimo/outreach/_ffidx_b93.json', encoding='utf-8'))
...
for code, v in IDX.items():
    ...
    if score:
        ...
        snip = v['content'][max(0, pos - 200):pos + 500].replace('\n', ' ')
        rows.append((score, CNT.get(code, 0), code, v['title'], snip))
rows.sort(key=lambda r: (r[1], -r[0]))
for score, used, code, title, snip in rows[:6]:
